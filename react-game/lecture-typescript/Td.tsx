import React, {useContext, useCallback, useMemo, memo, FC, MouseEvent} from 'react';
import { CODE, TableContext, Codes } from './MineSearch';
...
const Td: FC<Props> = memo(({ rowIndex, cellIndex }) => {
    ...
    const onRightClickTd = useCallback((e: MouseEvent) => {
        e.preventDefault();
        if (halted) {
            return;
        }
...
interface RealTdProps {
    onClickTd: () => void;
    onRightClickTd: (e: MouseEvent) => void;
    data: Codes;
}
